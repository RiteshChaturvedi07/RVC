-- Emergency repair for projects where the earlier lifecycle migration was not
-- applied completely. Safe to run once in the Supabase SQL editor.
...
do $$ declare c record; begin
  for c in select conname from pg_constraint where conrelid='public.restaurant_orders'::regclass and contype='c' and pg_get_constraintdef(oid) ilike '%status%'
  loop execute format('alter table public.restaurant_orders drop constraint if exists %I',c.conname); end loop;
end $$;
...
-- Replace any incomplete five-argument public-order RPC with the complete one.
...
returns jsonb language sql security definer set search_path=public as $$
 select jsonb_build_object(
  'restaurant',jsonb_build_object('name',coalesce(s.display_name,t.name),'currency',coalesce(s.currency,'INR'),'tax_rate',coalesce(s.tax_rate,0),'merchant_upi_id',s.merchant_upi_id,'merchant_upi_qr_url',s.merchant_upi_qr_url),
