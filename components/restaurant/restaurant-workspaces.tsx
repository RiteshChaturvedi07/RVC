...
export function RestaurantKitchenBoard(){const state=useOrders();const update=async(o:Order)=>{const next=o.status==='new'?'accepted':o.status==='accepted'?'preparing':o.status==='preparing'?'ready':'served';const{error}=await db().from('restaurant_orders').update({status:next,updated_at:new Date().toISOString()}).eq('id',o.id);if(error)alert(error.message);else state.refresh()};if(!state.data)return <Busy error={state.error}/>;const active=state.data.filter(o=>!['served','completed','closed','cancelled'].includes(o.status));return <div className="space-y-6"><Header title="Kitchen display" description="A live, focused queue for your kitchen team."/><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{active.map(o=><article key={o.id} className="rounded-2xl border border-border bg-card p-5"><div className="flex justify-between"><b className="text-xl">#{o.order_number}</b><span className="capitalize text-sm text-primary">{o.status}</span></div><div className="mt-3"><Guest order={o}/></div><p className="mt-4 text-sm">{o.restaurant_order_items.map(i=>`${i.item_name} ×${i.quantity}`).join(', ')}</p><button onClick={()=>update(o)} className="mt-6 w-full rounded-xl bg-primary py-3 text-sm font-semibold text-primary-foreground">{o.status==='ready'?'Mark served':o.status==='preparing'?'Mark ready':'Start preparing'}</button></article>)}</div></div>}
